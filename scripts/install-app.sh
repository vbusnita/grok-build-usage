...
# Launcher — fully transparent to Finder; logs to ~/Library/Logs
...
if [[ ! -x "\${PYTHON_BIN}" ]]; then
  osascript -e 'display notification "Grok Build Usage venv missing — re-run install-app.sh" with title "Grok Build Usage"' 2>/dev/null || true
  echo "missing python: \${PYTHON_BIN}" >>"\${LOG_FILE}"
  exit 1
fi

# Single-instance: if already running, bring attention via notification
if pgrep -f "python.*-m gbu" >/dev/null 2>&1; then
  osascript -e 'display notification "Already running in the menu bar" with title "Grok Build Usage"' 2>/dev/null || true
  exit 0
fi

export PYTHONUNBUFFERED=1
# Detach from Finder launch context; keep running after parent exits
cd "\${PROJECT_DIR}"
exec >>"\${LOG_FILE}" 2>&1
echo "---- \$(date -u +%Y-%m-%dT%H:%M:%SZ) start ----"
exec "\${PYTHON_BIN}" -m gbu
EOF
